...
There are a few add-ons to this algorithm for controlling a robot in practice. The algorithm needs pedestrian positions and velocities as well as the current robot position every tick. It uses this to simulate `n_samples` paths forward (of `n_steps` timesteps) for each pedestrian using a Gaussian process parameterized by the agent's intent and flexibility -- which are also tunable parameters. The robot is modeled as a differential drive system and its samples are generated in the control space in order to preserve the dynamics of the system. After the BRNE optimization, the controls that lead to the optimal robot path are published. The trajectory plan should be updated at a goal rate of 10 Hz. 
...
My C++ implementation of the algorithm uses the Armadillo library for the matrix operations of the BRNE algorithm as well as OpenMP for parallelization. This is comparable with the Numba version of the algorithm for small numbers of pedestrians, but does not see the massive spike that the Python implementation does after 4 agents. The C++ algorithm is better suited for this purpose and also scales more efficiently if more agents need to be considered in the future. 
...
An important part of this system that carried over to my future designs was the configuration of the WiFi hotspot onboard the Unitre's Raspberrry Pi. Alongside Matthew Elwin, I set up the hotspot to behave as a bridge between WiFi connections (to my computer) and ethernet connections (to the other Jetson boards inside the robot). This allowed me to run ROS 2 nodes on any device in this network and made the system truly wireless. 
...
*  Have the BRNE algorithm running onboard the Orin Nano or another mounted device such as the more powerful Orin AGX. Using CUDA could help optimize the runtime even more. 
*  Configure the WiFi hotspot to be from the Orin Nano instead of the Unitree. The Unitree hotspot is flaky and can easily get overloaded as it is broadcased from the onboard Raspberry Pi. A hotspot provided by the Orin might be more robust.
*  Adjust the robot's sampling. Currently, the robot is modeled as a simple differential drive system and the samples are generated over a space of linear velocity and angular velocity. First, the Unitree robot is omnidirectional, and the controls can consist of $$v_x$$, $$v_y$$, and $$v_\theta$$. This may make the robot more agile. It also may be beneficial to consider controls that can vary over the planning horizon as opposed to only considering constant linear and angular velocity. 
